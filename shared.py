...
def load_pcb_from_c(so_path: str) -> PCB:
    ...
    return PCB(
        pid=c_struct.pid, user_id=c_struct.user_id,
        role=Role(c_struct.role), job_type=JobType(c_struct.job_type),
        deadline=c_struct.deadline, cpu_budget_ns=c_struct.cpu_budget_ns,
        urgency_score=c_struct.urgency_score, abuse_flag=bool(c_struct.abuse_flag),
        state=State(c_struct.state), cpu_used=c_struct.cpu_used
    )